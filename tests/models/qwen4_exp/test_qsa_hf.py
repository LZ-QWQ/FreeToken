...
def test_single_layer_matches_hf_reference(monkeypatch):
    ...
    if torch.version.hip is None:
        ...
        # ReLU can leave exact-zero blocks at the cutoff. torch.topk and the
        # Triton selector may choose different IDs from that tie, so inspect
        # only rows below the existing Jaccard threshold.
